...
class PurchaseController extends GetxController {
  ...
  Future getSummary(String? id) async {
    Response response = await api.getData("api/Dashboard/PurchaseSummary",query: {"organizationId":id});
    print(response.statusCode);
    if (response.statusCode == 200) {
      // If the server did return a 201 CREATED response,
      // then parse the JSON.
      summary =  purchaseSummary.fromJson(response.body);

      print(summary?.CompletedPurchaseInvoice);
      update();
    } else {
